...
@implementation PBDetailIndustryData
...
+(NSMutableArray *)searchInducstry:(int)num
{
    ...
    const char *sql = [[NSString stringWithFormat:@"%@",@"select kind,id,name from kbn_master  where kind = ? "]UTF8String];
    ...
    NSString *industry = [NSString stringWithFormat:@"industry%d",num];
    
    if (sqlite3_prepare_v2(database, sql, -1, &stmt, NULL) == SQLITE_OK) {
       
        sqlite3_bind_text(stmt, 1, [industry UTF8String],-1, SQLITE_TRANSIENT);
        
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            PBDetailIndustryData* data = [[[PBDetailIndustryData alloc]init] autorelease];  
