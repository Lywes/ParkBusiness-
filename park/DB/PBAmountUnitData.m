...
@implementation PBAmountUnitData
...
+(NSMutableArray*)search
{
    ...
    const char *sql = [[NSString stringWithFormat:@"%@",@"select kind,id,name from kbn_master  where kind = 'unit'"]UTF8String];
    ...
    if (sqlite3_prepare_v2(database, sql, -1, &stmt, NULL) == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            PBAmountUnitData* data = [[[PBAmountUnitData alloc]init] autorelease];  
