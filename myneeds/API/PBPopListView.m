...
- (void)defalutInit
{
    ...
    CGRect tableFrame = CGRectMake(0, 32.0f, xWidth, self.bounds.size.height-82.0f);
    _listView = [[UITableView alloc] initWithFrame:tableFrame style:UITableViewStylePlain];
    _listView.dataSource = self;
    _listView.delegate = self;
    [self addSubview:_listView];
    self.backgroundColor = _listView.backgroundColor;
    ...
    [submitBtn setBackgroundImage:[UIImage imageNamed:@"custom_button.png"] forState:UIControlStateNormal];
    [submitBtn setTitle:@"确定" forState:UIControlStateNormal];
    submitBtn.titleLabel.textAlignment = UITextAlignmentCenter;
    submitBtn.frame = CGRectMake(0, 0, 255, 50);
...
-(void)submit{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(submitBtnDidPush)]) {
        [self.delegate submitBtnDidPush];
    }
    [self fadeOut];
}
...
{
    if(self.datasource &&
       [self.datasource respondsToSelector:@selector(popoverNumberOfSectionsListView:)])
    {
        return [self.datasource popoverNumberOfSectionsListView:self];
    }
...
- (void)show
{
    ...
    submitBtn.center = CGPointMake(self.frame.size.width/2,self.frame.size.height-submitBtn.frame.size.height/2);
    [self fadeIn];
}
