...
- (id)initWithPlaceHolder:(NSString*)placeHolder maxCount:(NSUInteger)maxCount
{
    ...
    if (self) {
        ...
        [self._closeButton setImage:[UIImage closeButtonImageWithSize:CGSizeMake(CLOSE_IMAGE_WIDTH, CLOSE_IMAGE_WIDTH) 
                                                     strokeColor:[UIColor whiteColor] 
                                                       fillColor:[UIColor blackColor] 
                                                          shadow:NO] 
                      forState:UIControlStateNormal];
        self._closeButton.frame = CGRectMake(0, 0, CLOSE_IMAGE_WIDTH, CLOSE_IMAGE_WIDTH);
        self._closeButton.showsTouchWhenHighlighted = YES;
        self._closeButton.titleLabel.font = [UIFont systemFontOfSize:COUNT_SIZE];
        [self._closeButton addTarget:self action:@selector(handleCloseButton:) forControlEvents:UIControlEventTouchUpInside];
        self._closeButton.frame = CGRectMake(_popupView.bounds.size.width-TEXTVIEW_INSETS.right-CLOSE_IMAGE_WIDTH, 
                                        0,
                                        CLOSE_BUTTON_WIDTH, 
                                        CLOSE_BUTTON_WIDTH);
        self._closeButton.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleBottomMargin;
        [_popupView addSubview:self._closeButton];
...
- (void)setShowCloseButton:(BOOL)showCloseButton
{
    ...
    self._closeButton.hidden = !showCloseButton;
}
...
- (void)didReceiveTextDidChangeNotification:(NSNotification*)notification
{
    ...
    [self.delegate popupTextView:self didChangeWithText:self.text];
    [self updateCount];
...
{
    [self dismiss];
    [self.delegate closePopTextView];
