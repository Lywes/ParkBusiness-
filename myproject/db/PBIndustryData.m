...
+(NSMutableArray*)search:(NSString *)kind1
{
    ...
    if (sqlite3_prepare_v2(database, sql, -1, &stmt, NULL) == SQLITE_OK) {
        sqlite3_bind_text(stmt, 1, [kind1 UTF8String],-1, SQLITE_TRANSIENT);
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            PBIndustryData* data = [[[PBIndustryData alloc]init] autorelease];  
